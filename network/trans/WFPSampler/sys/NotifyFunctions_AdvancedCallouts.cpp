...
//
//   Module Name:
//      NotifyFunctions_AdvancedCallouts.cpp
//
//   Abstract:
//      This module contains WFP Notify functions for the simpler callouts.
//
//   Naming Convention:
//
//      <Module><Scenario>
...
VOID PrvAdvancedNotificationWorkItemRoutine(_In_ PDEVICE_OBJECT pDeviceObject,
	...
	if (pWorkItemData)
	{
		...
		if (status != STATUS_SUCCESS)
		{
			if (status != STATUS_FWP_CALLOUT_NOT_FOUND)
				DbgPrintEx(DPFLTR_IHVNETWORK_ID,
					DPFLTR_ERROR_LEVEL,
					" !!!! [FilterID: %#I64x][CalloutID: %#x] PrvAdvancedNotificationWorkItemRoutine : FwpmCalloutGetById() [status: %#x]\n",
...
#pragma warning(push)
#pragma warning(disable: 6014) /// pNotifyData is expected to be cleaned up by caller using PrvAdvancedNotificationWorkItemRoutine
