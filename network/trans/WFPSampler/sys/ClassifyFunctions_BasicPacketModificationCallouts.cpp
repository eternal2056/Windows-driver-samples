	...
	_Inout_ FWPS_CLASSIFY_OUT* pClassifyOut)
{
	...
	if (pClassifyOut->rights & FWPS_RIGHT_ACTION_WRITE)
	{
		...
		if (pNetBufferList)
		{
			...
			////获取流数据io packet的指针
			FWPS_STREAM_CALLOUT_IO_PACKET0* streamPacket = (FWPS_STREAM_CALLOUT_IO_PACKET0*)pNetBufferList;
			////得到数据流指针
			...
			DbgPrint("inspect!In MonitorNfNotifyMessage(), nCurPid:%d localIP:%d:%d remoteIP:%d:%d ipProto:%d streamLength:%d stream:%s",
				"flowData->nCurPid",
